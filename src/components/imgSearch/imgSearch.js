...
class ImageSearch extends React.Component {
  ...
  render(){
    const { randomImg, searchResults } = this.state
    return(
      <div id="imgSearch">
        {console.log(this.state)}
        <div id="imgSearchTitle"><i className="fas fa-camera-retro titleIcon"></i> Image Search</div>
        <div>
          {this.renderRandomImg()}
        </div>
        <button className="button" onClick={this.getRandomImg}>Get a random image</button>
        <ImgGrid searchResults={searchResults}/>
